...
Shader "Hidden/Unlit/ItemIcon Shadow (TextureClip)_Alpha_ETC1"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
			    ...
			    half _alpha = tex2D(_ClipTex, i.clipUV).a;
				outcolor.a *= _alpha;
				outcolor.rgb = lerp(half3(0.0, 0.0, 0.0),outcolor.rgb, _alpha);
				return outcolor;
			}
			ENDCG
		}
		...
		Pass
		{
			...
			fixed4 frag (v2f IN) : COLOR
			{
				...
				half _alpha = tex2D(_ClipTex, IN.clipUV).a;
				outcolor.a *= _alpha;
				outcolor.rgb = lerp(half3(0.0, 0.0, 0.0),outcolor.rgb, _alpha);
				return outcolor;
			}
